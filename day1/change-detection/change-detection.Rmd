...
- If the probed item is in memory the participant responds correctly. If it isn't, they must guess:
    - $p(\mbox{resp} = \mbox{change} \mid \mbox{change}) = h =  d + (1 - d)g$
    - $p(\mbox{resp} = \mbox{no-change} \mid \mbox{change}) = f = (1 - d)g$
- This gives a formula for $k$:
    - $h = k/N + f \rightarrow k = N(h - f)$
...
$$
p(\mbox{resp} = \mbox{no-change} \mid \mbox{change}) = f = a(1 - d)g + (1 - a)g
$$
...
*Note* were only look at the data from the 50% change condition to simplify things.
